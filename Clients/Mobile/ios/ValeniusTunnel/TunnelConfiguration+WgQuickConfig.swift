...
//
// wg-quick config parser for TunnelConfiguration.
//
// NOTE: The `WireGuardKit` SwiftPM product does NOT ship the wg-quick string
// parser — `TunnelConfiguration(fromWgQuickConfig:)` lives in the WireGuard app's
// `Shared` sources, not the library, in every version. Every SPM consumer must
// therefore provide it. This file is the upstream parser
// (Sources/Shared/Model/TunnelConfiguration+WgQuickConfig.swift), vendored here so
// the Packet Tunnel extension can turn the wg-quick text passed via
// `providerConfiguration` into a TunnelConfiguration. Only WireGuardKit public
// types are used.
